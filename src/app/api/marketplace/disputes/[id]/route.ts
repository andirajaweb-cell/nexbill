...
import { tanggapiAduan } from "@/lib/marketplace/trust-service";
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json(await tanggapiAduan(id, session.outletId, body.statement, body.evidenceUrls, session.sub));
  } catch (err: unknown) {
