...
/** Menutup (menarik) barang dari etalase. Tidak menghapus baris — riwayat kesepakatan yang merujuknya harus tetap terbaca. */
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const row = await closeListing(id, session.outletId, session.sub);
