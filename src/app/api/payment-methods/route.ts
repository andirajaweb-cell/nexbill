...
export async function POST(req: NextRequest) {
  try {
    ...
    // (`presetKey` used to let the now-removed "Aktifkan Kanal iPaymu" quick-add panel create a
    // row with an exact, known-safe iPaymu channel key instead of relying on slugifyMethodKey —
    // see dashboard/payments/page.tsx's doc comment on why that panel was removed 2026-09-13.
    // Every payment method is now created purely from its label.)
