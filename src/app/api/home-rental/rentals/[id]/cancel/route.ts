...
import { cancelHomeRentalRental, markHomeRentalNoShow } from "@/lib/home-rental/rentals";
import { db } from "@/db/client";
import { homeRentalRentals } from "@/db/schema";
import { eq } from "drizzle-orm";

/** Body: { reason?, noShow?: boolean }. */
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const row = body?.noShow
      ? await markHomeRentalNoShow(id, session.sub)
      : await cancelHomeRentalRental(id, body?.reason || "Dibatalkan oleh staf", session.sub);
