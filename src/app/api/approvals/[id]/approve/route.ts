...
import { approveRequest } from "@/lib/pos/void";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!hasPermission(session.role as StaffRole, "approve_requests")) {
    return NextResponse.json({ error: "Role kamu tidak punya izin menyetujui permintaan." }, { status: 403 });
  }
  ...
  try {
    return NextResponse.json(await approveRequest(id, session.sub, session.role as StaffRole, note));
  } catch (err: unknown) {
