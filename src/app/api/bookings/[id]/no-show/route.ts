...
import { markNoShow } from "@/lib/rental/bookings";
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!hasPermission(session.role as StaffRole, "manage_bookings")) {
      return NextResponse.json({ error: "Role kamu tidak punya izin menandai no-show." }, { status: 403 });
    }
    ...
    return NextResponse.json(await markNoShow(id, session.sub));
  } catch (err: unknown) {
