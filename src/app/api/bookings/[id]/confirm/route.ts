...
import { confirmBooking } from "@/lib/rental/bookings";
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!hasPermission(session.role as StaffRole, "manage_bookings")) {
      return NextResponse.json({ error: "Role kamu tidak punya izin mengkonfirmasi booking." }, { status: 403 });
    }
    ...
    return NextResponse.json(await confirmBooking(id, session.sub));
  } catch (err: unknown) {
