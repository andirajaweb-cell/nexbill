...
export const outlets = sqliteTable("outlets", {
  ...
  expenseApprovalThreshold: real("expense_approval_threshold").notNull().default(500000),
  ...
  notifyPendingApproval: integer("notify_pending_approval", { mode: "boolean" }).notNull().default(true),
  notifyShiftVariance: integer("notify_shift_variance", { mode: "boolean" }).notNull().default(true),
  notifyBookingReminder: integer("notify_booking_reminder", { mode: "boolean" }).notNull().default(true),
  ...
  acceptOnlineBooking: integer("accept_online_booking", { mode: "boolean" }).notNull().default(true),
  ...timestamps,
});
 ...
 */
export const relayAgents = sqliteTable("relay_agents", {
  id: id(),
  outletId: text("outlet_id").notNull().references(() => outlets.id),
  name: text("name").notNull(),
 ...
 */
export const sessionAccessories = sqliteTable("session_accessories", {
  id: id(),
...
export const orders = sqliteTable("orders", {
  ...
  applyTax: integer("apply_tax", { mode: "boolean" }).notNull().default(false),
  applyServiceCharge: integer("apply_service_charge", { mode: "boolean" }).notNull().default(false),
  voucherId: text("voucher_id"),
  promoId: text("promo_id").references(() => promos.id),
  staffUserId: text("staff_user_id").references(() => staffUsers.id),
  shiftId: text("shift_id"),
...
export const agentSettings = sqliteTable("agent_settings", {
  ...
  isWhatsappEnabled: integer("is_whatsapp_enabled", { mode: "boolean" }).notNull().default(true),
  isInstagramEnabled: integer("is_instagram_enabled", { mode: "boolean" }).notNull().default(true),
 ...
 */
export const tuyaSettings = sqliteTable("tuya_settings", {
  id: id(),
  outletId: text("outlet_id").notNull().references(() => outlets.id),
...
export const accounts = sqliteTable("accounts", {
  ...
  isPostingAllowed: integer("is_posting_allowed", { mode: "boolean" }).notNull().default(true),
 ...
 */
export const ppobTransactions = sqliteTable("ppob_transactions", {
  id: id(),
  outletId: text("outlet_id").notNull().references(() => outlets.id),
  category: text("category", {
  ...
  customerId: text("customer_id").references(() => customers.id),
  customerName: text("customer_name"),
  nominal: real("nominal").notNull().default(0), // nilai/face value produk
  modal: real("modal").notNull().default(0), // uang keluar sebagai modal (harga beli/cost of the product itself)
  providerFee: real("provider_fee").notNull().default(0), // biaya dari principal/provider (Fastpay Fee Outlet, tier Basic) — beban riil, BUKAN pendapatan toko
  feeAdmin: real("fee_admin").notNull().default(0), // margin — keuntungan bersih toko, diatur sendiri (bukan dari tabel fee Fastpay)
  uangMasuk: real("uang_masuk").notNull().default(0), // = modal + providerFee + feeAdmin, total yang dibayar pelanggan
 ...
 */
export const ppobPriceRules = sqliteTable("ppob_price_rules", {
  id: id(),
  outletId: text("outlet_id").notNull().references(() => outlets.id),
  category: text("category", {
    enum: ["ewallet_topup", "token_listrik", "pulsa", "transfer", "tarik_tunai", "lainnya"],
  ...
  providerFee: real("provider_fee").notNull().default(0),
  defaultMargin: real("default_margin").notNull().default(0),
  notes: text("notes"),
  isActive: integer("is_active", { mode: "boolean" }).notNull().default(true),
  ...timestamps,
});
...
export const expenses = sqliteTable("expenses", {
  ...
  qty: integer("qty").notNull().default(1),
  amount: real("amount").notNull(), // total nominal (not unit price)
  ...
  recordAsPayable: integer("record_as_payable", { mode: "boolean" }).notNull().default(false),
  ...
  isRecurringInstance: integer("is_recurring_instance", { mode: "boolean" }).notNull().default(false),
...
export const assetDepreciationEntries = sqliteTable("asset_depreciation_entries", {
  id: id(),
...
export const recipeIngredients = sqliteTable("recipe_ingredients", {
  ...
  qtyPerYield: real("qty_per_yield").notNull(),
  unit: text("unit").notNull().default("pcs"),
});
...
export const membershipTiers = sqliteTable("membership_tiers", {
  ...
  outletId: text("outlet_id").notNull().references(() => outlets.id),
  name: text("name").notNull(),
  minSpending: real("min_spending").notNull().default(0),
  pointMultiplier: real("point_multiplier").notNull().default(1),
 ...
 */
export const loyaltyPlayPointRates = sqliteTable(
  "loyalty_play_point_rates",
  {
    id: id(),
    outletId: text("outlet_id").notNull().references(() => outlets.id),
    consoleType: text("console_type", { enum: ["ps2", "ps3", "ps4", "ps4_pro", "ps5", "ps5_slim"] }).notNull(),
    pointsPerSession: real("points_per_session").notNull().default(0),
    ...timestamps,
  },
...
export const loyaltyRewards = sqliteTable("loyalty_rewards", {
  ...
  pointsCost: real("points_cost").notNull(),
...
export const bookings = sqliteTable("bookings", {
  ...
  dpPaid: integer("dp_paid", { mode: "boolean" }).notNull().default(false),
  notes: text("notes"),
  cancelReason: text("cancel_reason"),
 ...
 */
export const bookingNotifications = sqliteTable("booking_notifications", {
  id: id(),
...
export const shifts = sqliteTable("shifts", {
  ...
  nonCashVarianceTotal: real("non_cash_variance_total"),
 ...
 */
export const shiftCashCounts = sqliteTable("shift_cash_counts", {
  id: id(),
  ...
  subtotal: real("subtotal").notNull().default(0), // denomination * qty, stored for audit even if the calc logic changes later
});
 ...
 */
export const shiftBalanceChecks = sqliteTable("shift_balance_checks", {
  id: id(),
  shiftId: text("shift_id").notNull().references(() => shifts.id),
 ...
 */
export const homeRentalProducts = sqliteTable("home_rental_products", {
  id: id(),
  outletId: text("outlet_id").notNull().references(() => outlets.id),
  name: text("name").notNull(),
  ...
  overnightRate: real("overnight_rate"),
  deliveryFee: real("delivery_fee").notNull().default(0),
  pickupFee: real("pickup_fee").notNull().default(0),
  defaultDepositAmount: real("default_deposit_amount").notNull().default(0),
  description: text("description"),
...
export const homeRentalAssets = sqliteTable(
  "home_rental_assets",
  {
    id: id(),
    outletId: text("outlet_id").notNull().references(() => outlets.id),
    ...
    isActive: integer("is_active", { mode: "boolean" }).notNull().default(true), // soft-delete/retire
    ...timestamps,
  },
  (t) => [uniqueIndex("home_rental_assets_outlet_code_idx").on(t.outletId, t.assetCode)]
);

export const homeRentalPackages = sqliteTable("home_rental_packages", {
  id: id(),
  outletId: text("outlet_id").notNull().references(() => outlets.id),
...
export const homeRentalPackageItems = sqliteTable("home_rental_package_items", {
  id: id(),
...
export const homeRentalRentals = sqliteTable("home_rental_rentals", {
  id: id(),
  outletId: text("outlet_id").notNull().references(() => outlets.id),
  ...
  verifiedKtp: integer("verified_ktp", { mode: "boolean" }).notNull().default(false),
  verifiedStudentId: integer("verified_student_id", { mode: "boolean" }).notNull().default(false),
  verifiedParentId: integer("verified_parent_id", { mode: "boolean" }).notNull().default(false),
  ...
  verifiedGetContact: integer("verified_get_contact", { mode: "boolean" }).notNull().default(false),
...
export const homeRentalRentalItems = sqliteTable("home_rental_rental_items", {
  id: id(),
...
export const homeRentalRentalAssets = sqliteTable("home_rental_rental_assets", {
  id: id(),
  rentalId: text("rental_id").notNull().references(() => homeRentalRentals.id),
 ...
 */
export const homeRentalCustomerRisk = sqliteTable(
...
export const subscriptionPlans = sqliteTable("subscription_plans", {
  ...
  name: text("name").notNull(),
  priceOriginal: real("price_original").notNull(), // 399000
  priceCurrent: real("price_current").notNull(), // 249000 (promo price actually billed)
  includedConsoles: integer("included_consoles").notNull().default(10),
  extraConsolePrice: real("extra_console_price").notNull().default(20000), // per console/bulan beyond includedConsoles
  smartPlugPrice: real("smart_plug_price").notNull().default(275000), // per unit hardware, billed separately from the plan itself
  setupServicePrice: real("setup_service_price").notNull().default(125000), // one-time, only if vendor-installed
  isActive: integer("is_active", { mode: "boolean" }).notNull().default(true),
  sortOrder: integer("sort_order").notNull().default(0),
  ...timestamps,
});
...
export const subscriptions = sqliteTable(
  "subscriptions",
  {
    ...
    aiLockedDuringTrial: integer("ai_locked_during_trial", { mode: "boolean" }).notNull().default(true),
...
export const subscriptionInvoices = sqliteTable("subscription_invoices", {
  ...
  qty: integer("qty").notNull().default(1),
  unitPrice: real("unit_price").notNull(),
  amount: real("amount").notNull(), // qty * unitPrice
 ...
 */
export const smartPlugOrders = sqliteTable("smart_plug_orders", {
  id: id(),
  outletId: text("outlet_id").notNull().references(() => outlets.id),
  subscriptionInvoiceId: text("subscription_invoice_id").notNull().references(() => subscriptionInvoices.id),
  qty: integer("qty").notNull().default(1),
  installRequested: integer("install_requested", { mode: "boolean" }).notNull().default(false), // false = self-install
