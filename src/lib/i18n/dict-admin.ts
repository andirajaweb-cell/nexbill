...
registerDict({
  ...
  "admin.reset.supabaseNote": {
    id: "Database di-host Supabase dan sudah punya backup otomatis (lihat Supabase Dashboard) — bukan file lokal yang dibuat aplikasi.",
    en: "The database is hosted on Supabase and already has automatic backups (see the Supabase Dashboard) — not a local file created by the app.",
    ms: "Pangkalan data dihoskan di Supabase dan sudah mempunyai sandaran automatik (lihat Supabase Dashboard) — bukan fail tempatan yang dicipta oleh aplikasi.",
    th: "ฐานข้อมูลโฮสต์อยู่บน Supabase และมีการสำรองข้อมูลอัตโนมัติอยู่แล้ว (ดูที่ Supabase Dashboard) — ไม่ใช่ไฟล์ในเครื่องที่แอปสร้างขึ้น",
    fil: "Ang database ay naka-host sa Supabase at mayroon nang automatic backup (tingnan ang Supabase Dashboard) — hindi ito local file na ginawa ng app.",
    vi: "Cơ sở dữ liệu được lưu trữ trên Supabase và đã có sao lưu tự động (xem Supabase Dashboard) — không phải tệp cục bộ do ứng dụng tạo ra.",
  },
  ...
  "admin.reset.doneMessage": {
    id: "Selesai. {n} tabel dikosongkan (hanya milik outlet ini). Pemulihan data mengandalkan backup otomatis Supabase (bukan file lokal). Kamu akan diarahkan ke halaman login.",
    en: "Done. {n} tables were cleared (belonging to this outlet only). Data recovery relies on Supabase's automatic backups (not a local file). You'll be redirected to the login page.",
    ms: "Selesai. {n} jadual telah dikosongkan (milik outlet ini sahaja). Pemulihan data bergantung pada sandaran automatik Supabase (bukan fail tempatan). Anda akan diarahkan ke halaman log masuk.",
    th: "เสร็จสิ้น ล้างข้อมูลไปแล้ว {n} ตาราง (เฉพาะของสาขานี้) การกู้คืนข้อมูลขึ้นอยู่กับการสำรองข้อมูลอัตโนมัติของ Supabase (ไม่ใช่ไฟล์ในเครื่อง) คุณจะถูกนำไปยังหน้าเข้าสู่ระบบ",
    fil: "Tapos na. {n} table ang na-clear (pag-aari lang ng outlet na ito). Ang pagbawi ng data ay umaasa sa automatic backup ng Supabase (hindi local file). Ire-redirect ka sa login page.",
    vi: "Hoàn tất. {n} bảng đã được xóa sạch (chỉ thuộc chi nhánh này). Việc khôi phục dữ liệu phụ thuộc vào bản sao lưu tự động của Supabase (không phải tệp cục bộ). Bạn sẽ được chuyển đến trang đăng nhập.",
  },
});
