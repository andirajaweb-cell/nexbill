...
registerDict({
  ...
  "admin.accessDenied": {
    id: "Halaman ini khusus Owner.",
    en: "This page is only for the Owner.",
    ms: "Halaman ini khusus untuk Owner.",
    th: "หน้านี้สำหรับ Owner เท่านั้น",
    fil: "Ang page na ito ay para lamang sa Owner.",
    vi: "Trang này chỉ dành cho Owner.",
  },
  "admin.staffNote": {
  ...
  "admin.ownerOnlyNote": {
    id: 'Akses penuh Admin Data (tambah/ubah/hapus tabel master) direservasi khusus untuk tim NEXBILL. Kamu login sebagai Owner — bagian "Hapus Semua Data" di bawah tetap bisa kamu akses.',
    en: "Full Admin Data access (add/edit/delete master tables) is reserved for the NEXBILL team. You're logged in as Owner — the \"Delete All Data\" section below is still available to you.",
    ms: 'Akses penuh Data Admin (tambah/ubah/padam jadual induk) dikhaskan untuk pasukan NEXBILL. Anda log masuk sebagai Owner — bahagian "Padam Semua Data" di bawah masih boleh anda akses.',
    th: 'สิทธิ์เข้าถึงข้อมูลผู้ดูแลระบบแบบเต็ม (เพิ่ม/แก้ไข/ลบตารางหลัก) สงวนไว้สำหรับทีม NEXBILL เท่านั้น คุณเข้าสู่ระบบในฐานะ Owner — ส่วน "ลบข้อมูลทั้งหมด" ด้านล่างยังคงใช้งานได้',
    fil: 'Ang buong access sa Admin Data (magdagdag/i-edit/tanggalin ang master table) ay nakalaan para sa team ng NEXBILL. Naka-login ka bilang Owner — ang seksyong "Tanggalin ang Lahat ng Data" sa ibaba ay puwede mo pa ring ma-access.',
    vi: 'Quyền truy cập đầy đủ vào Dữ liệu Admin (thêm/sửa/xóa bảng gốc) được dành riêng cho đội ngũ NEXBILL. Bạn đang đăng nhập với vai trò Owner — phần "Xóa toàn bộ dữ liệu" bên dưới vẫn có thể truy cập được.',
  },
  ...
  // --- PPOB orphaned-journal cleanup section ---
  "admin.ppobCleanup.title": { id: "Bersihkan Jurnal PPOB Yatim (Perbaikan Saldo Minus)", en: "Clean Up Orphaned PPOB Journal Entries (Negative Balance Fix)", ms: "Bersihkan Jurnal PPOB Yatim (Pembetulan Baki Negatif)", th: "ล้างรายการบัญชี PPOB ที่ตกค้าง (แก้ยอดติดลบ)", fil: "Linisin ang Orphaned na PPOB Journal Entries (Ayusin ang Negatibong Balanse)", vi: "Dọn bút toán PPOB mồ côi (sửa số dư âm)" },
  "admin.ppobCleanup.desc": { id: 'Kalau kamu pernah melihat saldo channel PPOB/non-tunai yang aneh (misalnya minus) padahal transaksinya sudah dihapus, itu kemungkinan sisa dari bug lama yang sudah diperbaiki. Klik "Cek Sekarang" untuk melihat apakah outlet ini terdampak, lalu "Bersihkan" untuk menghapus sisa-sisanya. Tidak menyentuh transaksi yang masih ada.', en: 'If you\'ve ever seen a strange PPOB/non-cash channel balance (e.g. negative) even though the transaction was already deleted, it\'s likely leftover from an old bug that\'s now fixed. Click "Check Now" to see if this outlet is affected, then "Clean Up" to remove the leftovers. This never touches a transaction that still exists.', ms: 'Jika kamu pernah lihat baki saluran PPOB/bukan tunai yang pelik (contohnya negatif) walaupun transaksi sudah dipadam, ia berkemungkinan baki daripada bug lama yang sudah dibetulkan. Klik "Semak Sekarang" untuk lihat jika outlet ini terjejas, kemudian "Bersihkan" untuk buang bakinya. Tidak menyentuh transaksi yang masih wujud.', th: "ถ้าคุณเคยเห็นยอดคงเหลือช่อง PPOB/ไม่ใช่เงินสดที่แปลก (เช่น ติดลบ) ทั้งที่ลบธุรกรรมไปแล้ว นั่นอาจเป็นเศษที่เหลือจากบั๊กเก่าที่แก้ไขแล้ว กด \"ตรวจสอบตอนนี้\" เพื่อดูว่าสาขานี้ได้รับผลกระทบหรือไม่ แล้ว \"ล้างข้อมูล\" เพื่อลบเศษที่เหลือ จะไม่แตะต้องธุรกรรมที่ยังมีอยู่", fil: 'Kung nakakita ka na ng kakaibang balanse sa PPOB/non-cash channel (hal. negatibo) kahit na tinanggal na ang transaksyon, malamang ito ay natira mula sa lumang bug na naayos na. I-click ang "Check Now" para tingnan kung apektado ang outlet na ito, tapos "Linisin" para tanggalin ang natira. Hindi ito ginagalaw ang transaksyong buhay pa.', vi: 'Nếu bạn từng thấy số dư kênh PPOB/phi tiền mặt bất thường (ví dụ âm) dù giao dịch đã bị xóa, đó có thể là tàn dư từ lỗi cũ đã được sửa. Nhấn "Kiểm tra ngay" để xem chi nhánh này có bị ảnh hưởng không, sau đó "Dọn dẹp" để xóa phần còn sót lại. Không đụng đến giao dịch nào còn tồn tại.' },
  "admin.ppobCleanup.checking": { id: "Mengecek...", en: "Checking...", ms: "Menyemak...", th: "กำลังตรวจสอบ...", fil: "Chinecheck...", vi: "Đang kiểm tra..." },
  "admin.ppobCleanup.checkBtn": { id: "Cek Sekarang", en: "Check Now", ms: "Semak Sekarang", th: "ตรวจสอบตอนนี้", fil: "Check Now", vi: "Kiểm tra ngay" },
  "admin.ppobCleanup.cleaning": { id: "Membersihkan...", en: "Cleaning up...", ms: "Membersihkan...", th: "กำลังล้างข้อมูล...", fil: "Nililinis...", vi: "Đang dọn dẹp..." },
  "admin.ppobCleanup.cleanBtn": { id: "Bersihkan {count} Entri", en: "Clean Up {count} Entries", ms: "Bersihkan {count} Entri", th: "ล้าง {count} รายการ", fil: "Linisin ang {count} Entry", vi: "Dọn {count} bút toán" },
  "admin.ppobCleanup.clean": { id: "Bersih — tidak ada entri jurnal yatim ditemukan.", en: "Clean — no orphaned journal entries found.", ms: "Bersih — tiada entri jurnal yatim ditemui.", th: "สะอาด — ไม่พบรายการบัญชีที่ตกค้าง", fil: "Malinis — walang nahanap na orphaned journal entries.", vi: "Sạch — không tìm thấy bút toán mồ côi nào." },
  "admin.ppobCleanup.foundCount": { id: "Ditemukan {count} entri jurnal yatim, berdampak pada akun:", en: "Found {count} orphaned journal entries, affecting these accounts:", ms: "Ditemui {count} entri jurnal yatim, memberi kesan kepada akaun:", th: "พบรายการบัญชีที่ตกค้าง {count} รายการ ส่งผลต่อบัญชี:", fil: "May nahanap na {count} orphaned journal entries, apektado ang mga account:", vi: "Tìm thấy {count} bút toán mồ côi, ảnh hưởng đến các tài khoản:" },
  "admin.ppobCleanup.confirmMessage": { id: "Hapus {count} entri jurnal PPOB yatim ini? Ini hanya menghapus entri yang transaksinya sudah tidak ada sama sekali — tidak menyentuh transaksi yang masih hidup.", en: "Delete these {count} orphaned PPOB journal entries? This only removes entries whose transaction no longer exists at all — it never touches a live transaction.", ms: "Padam {count} entri jurnal PPOB yatim ini? Ini hanya memadam entri yang transaksinya sudah tiada sama sekali — tidak menyentuh transaksi yang masih wujud.", th: "ลบรายการบัญชี PPOB ที่ตกค้าง {count} รายการนี้หรือไม่? จะลบเฉพาะรายการที่ธุรกรรมไม่มีอยู่แล้วเท่านั้น — จะไม่แตะต้องธุรกรรมที่ยังมีอยู่", fil: "Tanggalin ang {count} orphaned na PPOB journal entries na ito? Tatanggalin lang nito ang mga entry na wala nang transaksyon sa likod — hindi ito ginagalaw ang buhay na transaksyon.", vi: "Xóa {count} bút toán PPOB mồ côi này? Chỉ xóa các bút toán mà giao dịch của chúng không còn tồn tại — không đụng đến giao dịch còn sống." },
  "admin.ppobCleanup.doneMessage": { id: "Selesai. {count} entri jurnal yatim dihapus — saldo yang terdampak sudah dikoreksi.", en: "Done. {count} orphaned journal entries removed — the affected balances are now corrected.", ms: "Selesai. {count} entri jurnal yatim dipadam — baki yang terjejas sudah dibetulkan.", th: "เสร็จสิ้น ลบรายการบัญชีที่ตกค้าง {count} รายการแล้ว — ยอดคงเหลือที่ได้รับผลกระทบได้รับการแก้ไขแล้ว", fil: "Tapos na. {count} orphaned journal entries ang natanggal — naitama na ang mga apektadong balanse.", vi: "Xong. Đã xóa {count} bút toán mồ côi — số dư bị ảnh hưởng đã được sửa." },
  ...
  "admin.reset.desc2Suffix": {
    id: " (akun Owner lain di outlet ini juga tetap ada). Kontrol Perangkat yang sudah kamu setting (device, TV, relay) juga tidak ikut terhapus. Semua akun staf non-Owner (Manager, Kasir, dll) di outlet ini ikut terhapus.",
    en: " (other Owner accounts at this outlet also remain). Device Control settings you've already set up (devices, TVs, relays) aren't deleted either. All non-Owner staff accounts (Manager, Cashier, etc.) at this outlet will be deleted too.",
    ms: " (akaun Owner lain di outlet ini juga kekal). Tetapan Kawalan Peranti yang telah anda konfigurasikan (peranti, TV, geganti) turut tidak dipadam. Semua akaun staf bukan Owner (Manager, Juruwang, dll) di outlet ini turut dipadam.",
    th: " (บัญชี Owner อื่นที่สาขานี้ก็จะยังคงอยู่) การตั้งค่าควบคุมอุปกรณ์ที่คุณตั้งไว้แล้ว (อุปกรณ์ ทีวี รีเลย์) ก็จะไม่ถูกลบเช่นกัน บัญชีพนักงานที่ไม่ใช่ Owner ทั้งหมด (ผู้จัดการ แคชเชียร์ ฯลฯ) ในสาขานี้จะถูกลบไปด้วย",
    fil: " (mananatili rin ang ibang Owner account sa outlet na ito). Hindi rin matatanggal ang mga Device Control setting na na-configure mo na (device, TV, relay). Lahat ng staff account na hindi Owner (Manager, Cashier, atbp.) sa outlet na ito ay matatanggal din.",
    vi: " (các tài khoản Owner khác tại chi nhánh này vẫn được giữ lại). Cài đặt Kiểm soát Thiết bị mà bạn đã cấu hình (thiết bị, TV, relay) cũng không bị xóa. Tất cả tài khoản nhân viên không phải Owner (Quản lý, Thu ngân, v.v.) tại chi nhánh này cũng sẽ bị xóa.",
  },
