...
registerDict({
  ...
  "devices.guide.tuya.body": {
    id: 'Untuk smart plug yang pakai aplikasi Tuya Smart / Smart Life. PENTING: outlet ini wajib punya akun Tuya Cloud API sendiri dulu (isi di Pengaturan > Integrasi Tuya Cloud API) — lihat langkah lengkapnya di Pusat Bantuan > Kontrol Perangkat. Setelah itu: 1) Pasang perangkat & hubungkan ke WiFi lewat aplikasi Tuya Smart / Smart Life seperti biasa. 2) Buka aplikasi itu, masuk ke detail perangkat > ikon pensil/Device Information, salin "Device ID"-nya. 3) Di sini, pilih protokol "Tuya Smart Life", tempel Device ID itu — kolom "Kode DP Switch" boleh dikosongkan (default switch_1). 4) Simpan dan coba tombol Nyalakan/Matikan.',
    en: 'For smart plugs that use the Tuya Smart / Smart Life app. IMPORTANT: this outlet must first have its own Tuya Cloud API account (fill it in at Settings > Tuya Cloud API Integration) — see the full steps in Help Center > Device Control. After that: 1) Set up the device and connect it to WiFi via the Tuya Smart / Smart Life app as usual. 2) Open that app, go to the device\'s details > pencil icon/Device Information, copy its "Device ID". 3) Here, choose the "Tuya Smart Life" protocol and paste that Device ID — the "Switch DP Code" field can stay blank (defaults to switch_1). 4) Save and try the Turn On/Turn Off buttons.',
    ms: 'Untuk smart plug yang menggunakan aplikasi Tuya Smart / Smart Life. PENTING: outlet ini wajib ada akaun Tuya Cloud API sendiri dahulu (isi di Tetapan > Integrasi Tuya Cloud API) — lihat langkah penuh di Pusat Bantuan > Kawalan Peranti. Selepas itu: 1) Pasang peranti & hubungkan ke WiFi melalui aplikasi Tuya Smart / Smart Life seperti biasa. 2) Buka aplikasi itu, masuk ke butiran peranti > ikon pensel/Device Information, salin "Device ID"-nya. 3) Di sini, pilih protokol "Tuya Smart Life", tampal Device ID itu — kolum "Kod DP Switch" boleh dibiarkan kosong (lalai switch_1). 4) Simpan dan cuba butang Hidupkan/Matikan.',
    th: 'สำหรับปลั๊กอัจฉริยะที่ใช้แอป Tuya Smart / Smart Life สำคัญ: ร้านนี้ต้องมีบัญชี Tuya Cloud API ของตัวเองก่อน (กรอกที่การตั้งค่า > การเชื่อมต่อ Tuya Cloud API) — ดูขั้นตอนเต็มที่ศูนย์ช่วยเหลือ > การควบคุมอุปกรณ์ จากนั้น: 1) ติดตั้งอุปกรณ์และเชื่อมต่อ WiFi ผ่านแอป Tuya Smart / Smart Life ตามปกติ 2) เปิดแอปนั้น ไปที่รายละเอียดอุปกรณ์ > ไอคอนดินสอ/Device Information คัดลอก "Device ID" 3) ที่นี่ เลือกโปรโตคอล "Tuya Smart Life" วาง Device ID นั้น — ช่อง "รหัส DP Switch" เว้นว่างได้ (ค่าเริ่มต้น switch_1) 4) บันทึกแล้วลองปุ่มเปิด/ปิด',
    fil: 'Para sa mga smart plug na gumagamit ng Tuya Smart / Smart Life app. MAHALAGA: dapat mayroon munang sariling Tuya Cloud API account ang outlet na ito (ilagay sa Settings > Tuya Cloud API Integration) — tingnan ang buong hakbang sa Help Center > Device Control. Pagkatapos noon: 1) I-setup ang device at ikonekta sa WiFi gamit ang Tuya Smart / Smart Life app gaya ng dati. 2) Buksan ang app na iyon, pumunta sa detalye ng device > pencil icon/Device Information, kopyahin ang "Device ID" nito. 3) Dito, piliin ang protocol na "Tuya Smart Life", i-paste ang Device ID na iyon — puwedeng iwanang blangko ang field na "Switch DP Code" (default switch_1). 4) I-save at subukan ang mga button na I-on/I-off.',
    vi: 'Dành cho ổ cắm thông minh dùng app Tuya Smart / Smart Life. QUAN TRỌNG: cửa hàng này phải có tài khoản Tuya Cloud API riêng trước (nhập tại Cài đặt > Tích hợp Tuya Cloud API) — xem đầy đủ các bước tại Trung tâm trợ giúp > Điều khiển thiết bị. Sau đó: 1) Thiết lập thiết bị và kết nối WiFi qua app Tuya Smart / Smart Life như bình thường. 2) Mở app đó, vào chi tiết thiết bị > biểu tượng bút chì/Device Information, sao chép "Device ID". 3) Ở đây, chọn giao thức "Tuya Smart Life", dán Device ID đó — ô "Mã DP Switch" có thể để trống (mặc định switch_1). 4) Lưu lại và thử nút Bật/Tắt.',
  },
  "devices.guide.tuya.activationNote": {
    id: 'Kalau tombol Nyalakan/Matikan belum berfungsi setelah Langkah 4, cek dulu: (1) sudah isi Access ID/Secret Tuya Cloud API outlet ini di Pengaturan? (2) langganan IoT Core di akun Tuya itu masih aktif (belum expired)? Kalau sudah benar tapi tetap gagal, hubungi tim NEXBILL lewat menu Chat/Bantuan.',
    en: 'If the Turn On/Off button still doesn\'t work after Step 4, first check: (1) has this outlet\'s Tuya Cloud API Access ID/Secret been filled in at Settings? (2) is that Tuya account\'s IoT Core subscription still active (not expired)? If both check out but it still fails, contact the NEXBILL team via Chat/Help.',
    ms: 'Jika butang Hidupkan/Matikan masih tidak berfungsi selepas Langkah 4, semak dahulu: (1) sudah isi Access ID/Secret Tuya Cloud API outlet ini di Tetapan? (2) langganan IoT Core akaun Tuya itu masih aktif (belum tamat)? Jika sudah betul tetapi masih gagal, hubungi pasukan NEXBILL melalui menu Chat/Bantuan.',
    th: 'หากปุ่มเปิด/ปิดยังไม่ทำงานหลังขั้นตอนที่ 4 ให้ตรวจสอบก่อนว่า: (1) กรอก Access ID/Secret ของ Tuya Cloud API ร้านนี้ที่การตั้งค่าแล้วหรือยัง? (2) การสมัคร IoT Core ของบัญชี Tuya นั้นยังใช้งานได้ (ยังไม่หมดอายุ) หรือไม่? หากถูกต้องแล้วแต่ยังล้มเหลว ติดต่อทีม NEXBILL ผ่านเมนู Chat/ช่วยเหลือ',
    fil: 'Kung hindi pa rin gumagana ang button na I-on/I-off pagkatapos ng Hakbang 4, unahin munang tingnan: (1) nalagay na ba ang Access ID/Secret ng Tuya Cloud API ng outlet na ito sa Settings? (2) aktibo pa ba ang subscription ng IoT Core ng Tuya account na iyon (hindi pa expired)? Kung tama na ang lahat pero nabigo pa rin, makipag-ugnayan sa team ng NEXBILL sa pamamagitan ng Chat/Help menu.',
    vi: 'Nếu nút Bật/Tắt vẫn không hoạt động sau Bước 4, hãy kiểm tra trước: (1) đã nhập Access ID/Secret Tuya Cloud API của cửa hàng này ở Cài đặt chưa? (2) gói IoT Core của tài khoản Tuya đó còn hiệu lực (chưa hết hạn) không? Nếu đã đúng mà vẫn lỗi, liên hệ đội ngũ NEXBILL qua menu Chat/Trợ giúp.',
  },
