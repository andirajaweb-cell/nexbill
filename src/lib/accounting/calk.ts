...
export async function buildCalk(outletId: string, from?: string, to?: string): Promise<Calk> {
  ...
  return {
    ...
    policies: [
      ...
      { title: "Persediaan", text: `Diukur pada biaya perolehan dengan metode ${outlet?.inventoryCostMethod === "fifo" ? "masuk pertama keluar pertama (FIFO)" : "rata-rata tertimbang"}${outlet?.inventoryCostMethodSince ? ` (berlaku sejak ${new Date(outlet.inventoryCostMethodSince).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })})` : ""}. Biaya perolehan meliputi harga beli ditambah ongkos angkut/parkir/biaya lain yang dibebankan pada pembelian. Selisih hasil stock opname dan barang rusak diakui sebagai beban pada periode terjadinya.` },
      { title: "Aset tetap", text: "Diakui sebesar biaya perolehan — harga beli ditambah biaya yang dapat diatribusikan langsung seperti ongkos kirim dan pemasangan — dan disusutkan dengan metode garis lurus selama umur manfaat masing-masing aset hingga nilai residunya. Keuntungan/kerugian pelepasan aset diakui dalam laba rugi." },
